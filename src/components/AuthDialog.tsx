...
const AuthDialog = ({ open, onOpenChange }: AuthDialogProps) => {
  ...
  const { signInWithGoogleSearchConsole } = useGoogleAuth();
  ...
  const handleGoogleSearchConsoleSignIn = async () => {
    try {
      setLoading(true);
      await signInWithGoogleSearchConsole();
      onOpenChange(false);
    } catch (error) {
      console.error('Errore durante l\'accesso con Search Console:', error);
      toast.error('Errore durante l\'accesso. Riprova.');
    } finally {
      setLoading(false);
    }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <Tabs defaultValue="login" className="w-full">
          ...
          <TabsContent value="login" className="space-y-4">
            ...
            <Button
              onClick={handleGoogleSearchConsoleSignIn}
              disabled={loading}
              variant="outline"
              size="lg"
